...
# Per quanto riguarda le bande, vengono prese le PRIME K-BANDE DA OGNI IMMAGINE DI INPUT
# Il numero di bande scelto è possibile specificarlo nella variabile "number_bands"
...
C_std_value<- 100 #Effettuare Tuning per valori specifici rispetto al numero di bande
gamma_std_value <- 10 #Effettuare Tuning per valori specifici rispetto al numero di bande
...
file_image <- writeRaster(final_image, filename=paste(final_dir,paste("file_image_KBANDS",ncol(datasetPrediction)-2,sep="-"),sep="/"), format="ENVI", bandorder='BIL', overwrite=TRUE)
