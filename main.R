...
#Numero di bande da prelevare come features
#ATTENZIONE a non superare il numero massimo
# !!USARE VALORE "-1" per selezionare TUTTE LE BANDE!!
number_bands <- 3
...
C_standard_value <- 1000 #consultare tabella relazione per ulteriori valori
gamma_standard_value <- 10 #consultare tabella relazione per ulteriori valori
...
checkDependencies()
#debug: (.packages()) per controllare se i pacchetti sono stati caricati correttamente
...
print("Creating initial Dataset (matrice)")
flush.console()
#----------------COSTRUZIONE DATASET------------------#
#Vedere file createDataset.R per ulteriori dettagli
tic_debug('Creating initial Dataset (matrice)')
matrice <- createDataset(dimX,dimY,imageList)
toc_debug()
...
#TEST: È interessante capire come il sistema si comporta al variare del numero di pixel di training
#Cioè mettere in relazione quanti dati di training dare ad SVM al fine di avere risultati soddisfacenti
...
if(tuning){
  ...
  tuned <- tune.svm(x=dati_training,
                    ...
                    kernel ="radial",
                    gamma = 2^(-5:5), cost = 2^(-5:5), #ACH: range MOLTO grande
                    tunecontrol = tune.control(cross = 10)) #cross validation
  print(tuned)
  #valori ottimali trovati
  C_value <- tuned$best.parameters$cost
  gamma_value <- tuned$best.parameters$gamma
  toc_debug()
}else{
  C_value <- C_standard_value
  gamma_value <- gamma_standard_value
}
...
print("Creazione matrice di confusione e Metriche")
flush.console()
tic_debug("Creazione matrice di confusione e Metriche")
...
#(x,y) con y = variabile predetta da SVM
...
file_image <- writeRaster(final_image, filename=paste("final_images/file_image",ncol(datasetPrediction)-2,sep=""), format="ENVI", bandorder='BIL', overwrite=TRUE)
