checkTrainingTest <- function(training_dir,test_dir){
  ...
  #quindi la libreria sceglie in maniera automatica il miglior modello svm, cioè quello che ha i migliori risultati
  #nella cross validation.
  #
  ...
  #Controllo SE il file "NO_DATA.shp" è presente in "training/" -> Mi serve saperlo
  ...
  #DEBUG: print(list_shapefile_test %in% list_shapefile)
  #DEBUG: print(all(list_shapefile_test %in% list_shapefile))
  #DEBUG: print(all(list_shapefile_test %in% list_shapefile))
  ...
  #ma mi basta controllare la lista "più lunga", cioè quella di "training/"
  if(no_data_present){
